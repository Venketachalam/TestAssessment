...
open class FakeDashboardFactory : NSObject {
 
 // open func getContracts(with data: DashBoardApiRequest) -> Observable<ContractsResponse>{
    open func getContracts(with data: DashBoardApiRequest){
...
//      filter.value = "0"
//
////      let payment : Dashboard = Dashboard()
...
//      contractsResponse.payload.dashboard = [stubbedJSON["Payment"] as! Dashboard]
//
